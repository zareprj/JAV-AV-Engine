...
#ifdef ALLOC_PRAGMA
#pragma alloc_text(PAGE, avl_create)
#pragma alloc_text(PAGE, avl_copy)
#pragma alloc_text(PAGE, avl_destroy)
#pragma alloc_text(PAGE, avl_probe)
#pragma alloc_text(PAGE, avl_insert)
#pragma alloc_text(PAGE, avl_replace)
#pragma alloc_text(PAGE, avl_delete)
#pragma alloc_text(PAGE, avl_find)
#pragma alloc_text(PAGE, avl_assert_insert)
#pragma alloc_text(PAGE, avl_assert_delete)
#pragma alloc_text(PAGE, avl_t_init)
#pragma alloc_text(PAGE, avl_t_first)
#pragma alloc_text(PAGE, avl_t_last)
#pragma alloc_text(PAGE, avl_t_find)
#pragma alloc_text(PAGE, avl_t_insert)
#pragma alloc_text(PAGE, avl_t_copy)
#pragma alloc_text(PAGE, avl_t_next)
#pragma alloc_text(PAGE, avl_t_prev)
#pragma alloc_text(PAGE, avl_t_cur)
#pragma alloc_text(PAGE, avl_t_replace)
#pragma alloc_text(PAGE, trav_refresh)
#pragma alloc_text(PAGE, copy_error_recovery)
#endif
...
#if defined(MIKTEX) && defined(__cplusplus)
  n = q->avl_link[dir] = (avl_node *)SMAlloc(sizeof *n);
    //tree->avl_alloc->libavl_malloc (tree->avl_alloc, sizeof *n);
#else
  n = q->avl_link[dir] =  (struct avl_node *) SMAlloc(sizeof *n);
	  //tree->avl_alloc->libavl_malloc (/*tree->avl_alloc, */ sizeof *n);
#endif
  if (n == NULL)
    return NULL;
...
copy_error_recovery (struct avl_node **stack, int height,
                     struct avl_table *newItem, avl_item_func *destroy)
{
  //assert (stack != NULL && height >= 0 && new != NULL);

  for (; height > 2; height -= 2)
    stack[height - 1]->avl_link[1] = NULL;
  avl_destroy (newItem/*, destroy*/);
}
          ...
          avl_item_func *destroy, struct libavl_allocator *allocator)
{
  ...
  struct avl_table *newItem;
  const struct avl_node *x;
  struct avl_node *y;

  //assert (org != NULL);
  newItem = avl_create (org->avl_compare, org->avl_param//,
                    /*allocator != NULL ? allocator : org->avl_alloc*/);
  if (newItem == NULL)
    return NULL;
  newItem->avl_count = org->avl_count;
  if (newItem->avl_count == 0)
    return newItem;

  x = (const struct avl_node *) &org->avl_root;
  y = (struct avl_node *) &newItem->avl_root;
  for (;;)
    {
      while (x->avl_link[0] != NULL)
        {
...
#if defined(MIKTEX) && defined(__cplusplus)
          y->avl_link[0] = (avl_node *) SMAlloc(sizeof *y->avl_link[0]);
            //new->avl_alloc->libavl_malloc (/*new->avl_alloc,*/
              //                             sizeof *y->avl_link[0]);
#else
          y->avl_link[0] =(struct avl_node *) SMAlloc(sizeof *y->avl_link[0]);
            //newItem->avl_alloc->libavl_malloc (/*newItem->avl_alloc,*/
                                           //sizeof *y->avl_link[0]);
#endif
          if (y->avl_link[0] == NULL)
            {
              if (y != (struct avl_node *) &newItem->avl_root)
                {
                  y->avl_data = NULL;
                  y->avl_link[1] = NULL;
                }

              copy_error_recovery (stack, height, newItem, destroy);
              return NULL;
            }
      ...
      for (;;)
        {
          ...
          if (x->avl_link[1] != NULL)
            {
#if defined(MIKTEX) && defined(__cplusplus)
              y->avl_link[1] = (avl_node *) SMAlloc(sizeof *y->avl_link[1]);
                //new->avl_alloc->libavl_malloc (/*new->avl_alloc,*/
                                               //sizeof *y->avl_link[1]);
#else
              y->avl_link[1] =(struct avl_node *) SMAlloc(sizeof *y->avl_link[1]);
                //newItem->avl_alloc->libavl_malloc (/*newItem->avl_alloc,*/
                                               //sizeof *y->avl_link[1]);
#endif
...
void
avl_destroy (struct avl_table *tree)
{
  struct avl_node *p, *q;
