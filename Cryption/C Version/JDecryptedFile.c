...
void CryptedFileInit(CryptFile *osCryptFile)
{
	...
	(*osCryptFile).Nr = 256 ;
	(*osCryptFile).Nb = 4;
	(*osCryptFile).Nk = (*osCryptFile).Nr / 32;
...
{
	UINT64 u64FileSize;
	if ( GetFileSizeEx ((*osCryptFile).hFile , ((LARGE_INTEGER*)&u64FileSize) ) == FALSE )
	{
		(*osCryptFile).LastErrorCode = GetLastError();
		return 0 ;
	}
	return u64FileSize;
}
...
BOOLEAN CryptOpen(CryptFile *osCryptFile,  TCHAR* szName)
{
	...
	(*osCryptFile).hFile  = CreateFile (szName, GENERIC_READ | GENERIC_WRITE , FILE_SHARE_READ , NULL , 
						   OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL ,NULL ) ;
	if ( (*osCryptFile).hFile == INVALID_HANDLE_VALUE ) 
	{
		(*osCryptFile).LastErrorCode = GetLastError();
		Iret = FALSE ;
	}
	//MessageBox(NULL, L"", L"", MB_OK | MB_ICONASTERISK);

	else
	{
		(*osCryptFile).FileSize = GetCryptFileLength(osCryptFile) ;
		Size = sizeof(UINT8) ;
		Iret = CryptRead(osCryptFile, &(osCryptFile->AlignByte), &Size);
		if (Iret == TRUE)
		{
...
BOOLEAN CryptCreateFile(CryptFile *osCryptFile, TCHAR* szName)
{
	...
	if ( (*osCryptFile).hFile == INVALID_HANDLE_VALUE ) 
	{
		...
		Size = sizeof(UINT8);
		Iret = CryptWrite(osCryptFile, &(osCryptFile->AlignByte), &Size);
		if (Iret == TRUE)
		{
			(*osCryptFile).RealFilePosition = (*osCryptFile).FilePosition   = 1LL ;
		}
...
//-----------------------------------------------------------------------------
BOOLEAN Seek(CryptFile *osCryptFile, INT64 *Offset, MoveMethod Method)
{
	BOOLEAN bResualt  = SetFilePointerEx (osCryptFile->hFile , *((LARGE_INTEGER*)Offset) , ((LARGE_INTEGER*)Offset) , Method);
	if (bResualt == FALSE )
	{
		(*osCryptFile).LastErrorCode = GetLastError ();
	}
...
BOOLEAN Read( CryptFile * osCryptFile, void * Data, size_t *TempSize ) 
{
	...
	BOOLEAN bRetValue = ReadFile(osCryptFile->hFile, Data , u32NumToRead, (LPDWORD)TempSize, NULL);
	if (!bRetValue || u32NumToRead != *TempSize)
	{
		osCryptFile->LastErrorCode = GetLastError();
		return FALSE;
	}
...
{
	UINT8 DiffPosition;
	//UINT32 DefDate = (UINT32)Data; 
	size_t TempSize = 0,IniSize , IniSize2, Temp  ;
	IniSize2 = IniSize = *Size = min ((*osCryptFile).FilePosition + *Size , (*osCryptFile).FileSize) - (*osCryptFile).FilePosition ;
	if ( (signed) Size < 0 ) 
	{
		return FALSE ;
	}
	...
	if ( (*osCryptFile).RealFilePosition != (*osCryptFile).FilePosition )
	{
		if ( (*osCryptFile).BufferPosition > (*osCryptFile).FilePosition || (*osCryptFile).BufferPosition + sizeof ((*osCryptFile).BUFFER) < (*osCryptFile).FilePosition )
		{
			...
			{
				Seek(osCryptFile, (INT64 *) (&(*osCryptFile).RealFilePosition), FBEGIN);
	...
	*Size = TempSize ;
	...
	/*	if ( !(((UINT32)Data - DefDate ) + TempSize <= IniSize2 ))
		{
			__asm int 3
		}*/
		*Size += TempSize ;
		if ( Temp > TempSize  )
		{
			GetCryptedFilePosition(osCryptFile, &(*osCryptFile).RealFilePosition) ;
		...
		while ( TempSize > 0)
		{
			(*osCryptFile).OutBuffer = (*osCryptFile).inBuffer = (BYTE *)Data  ; 
			InvCipher(osCryptFile); 
			Data  = (PBYTE)Data + sizeof((*osCryptFile).BUFFER) ;
	...
	if (TempSize != 0)
	{
		...
		(*osCryptFile).OutBuffer = (*osCryptFile).inBuffer = (BYTE *) (*osCryptFile).BUFFER ;
		InvCipher(osCryptFile); 
		(*osCryptFile).BufferPosition = (*osCryptFile).RealFilePosition ;
...
/*	if ( !(((UINT32)Data - DefDate )+TempSize == IniSize2 &&  IniSize2 == *Size ))
	{
		__asm int 3
	}*/
	(*osCryptFile).FilePosition += *Size;
...
{
	UINT32  u32NumToRead = *TempSize;
	BOOLEAN bRetValue = WriteFile( osCryptFile->hFile, (*osCryptFile).WriteBuffer , u32NumToRead , (LPDWORD)TempSize, NULL);
	if (!bRetValue || u32NumToRead != *TempSize)
	{
		osCryptFile->LastErrorCode = GetLastError();
		return FALSE;
...
BOOLEAN CryptWrite(CryptFile *osCryptFile, void *Data,DWORD *Size)
{
	...
	if ( (*osCryptFile).RealFilePosition != (*osCryptFile).FilePosition )
	{
		if ( (*osCryptFile).WriteBufferPosition > (*osCryptFile).FilePosition || (*osCryptFile).WriteBufferPosition + sizeof ((*osCryptFile).BUFFER) < (*osCryptFile).FilePosition )
		{
			...
			{
				if ( Seek( osCryptFile, (INT64 *) (&(*osCryptFile).WriteBufferPosition) , FBEGIN) == FALSE ) 
				{
					return FALSE ;
				}
				(*osCryptFile).OutBuffer = (*osCryptFile).inBuffer = (BYTE *)(*osCryptFile).WriteBuffer ;
				Cipher(osCryptFile);
				...
				if ( Seek( osCryptFile, (INT64 *) (&(*osCryptFile).RealFilePosition) , FBEGIN) == FALSE ) 
				{
					return FALSE ;
				}
			}
		    ...
		    (*osCryptFile).RealFilePosition += (*osCryptFile).WriteBufferPosition ;
			(*osCryptFile).OutBuffer = (*osCryptFile).inBuffer = (BYTE *)(*osCryptFile).WriteBuffer ;
			InvCipher(osCryptFile);
		}
		DiffPosition = (((UINT8)((*osCryptFile).FilePosition) + 1)) - (*osCryptFile).WriteBufferPosition ;
		TempSize = min ( (sizeof((*osCryptFile).BUFFER) - DiffPosition) , *Size  );
	...
	if (TempSize != 0 )
	{
		...
		if ( Write( osCryptFile, (PBYTE)Data - Temp , &Temp) == FALSE ) 
		{
			Size = 0 ;
			return FALSE ;
		}
...
BOOLEAN CryptClose(CryptFile *osCryptFile)
{
	...
	if ( (*osCryptFile).WriteBufferPosition != 0xffffffffffffffff )
	{
		...
		TempSize = min (  ((*osCryptFile).FileSize - (*osCryptFile).WriteBufferPosition) , sizeof ((*osCryptFile).WriteBuffer))  ;
		(*osCryptFile).WriteBufferPosition = 0 ;
		if ( Seek( osCryptFile, (INT64 *) (&(*osCryptFile).WriteBufferPosition) , FBEGIN) == FALSE ) 
		{
			return FALSE ;
		}
		(*osCryptFile).WriteBuffer[0] = TempSize ;
		TempSize = 1 ;
		if ( Write( osCryptFile, (*osCryptFile).WriteBuffer, &TempSize) == FALSE ) 
		{
			return FALSE ;
	...
	return CloseHandle((*osCryptFile).hFile); 
}
//-----------------------------------------------------------------------------
BOOLEAN  CryptSeek(CryptFile *osCryptFile, INT64 *Offset, MoveMethod Method)
{
	INT64 TempOffset;
	switch(Method) 
	{
	case FCURRENT :
		...
		break ;
	case FEND:
		(*osCryptFile).FilePosition = GetCryptFileLength(osCryptFile) - *Offset;
		break ;
	case FBEGIN:
		(*osCryptFile).FilePosition = *Offset;
		break ;
	...
	TempOffset = ((*osCryptFile).FilePosition) & 0xFFFFFFF0 ;
	TempOffset += 1 ;
	if ( Seek(osCryptFile, &TempOffset , FBEGIN) == FALSE ) 
	{
		return FALSE ;
	}
/*	if ( Seek(osCryptFile, Offset , Method) == FALSE ) 
	{
		return FALSE ;
	}*/
	(*osCryptFile).RealFilePosition = TempOffset ;
	*Offset = (*osCryptFile).FilePosition ;
	return TRUE;
}
//-----------------------------------------------------------------------------
void CryptKeyExpansion(CryptFile *osCryptFile) 
{
	...
	while (i < ((*osCryptFile).Nb * ((*osCryptFile).Nr+1)))
	{
		for(j=0;j<4;j++)
		{
